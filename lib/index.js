...
MapboxGeocoder.prototype = {
  ...
  addTo: function (container) {
    ...
    function addToExistingContainer(geocoder, container) {
      ...
      if (container.dataset && container.dataset.geocoderTarget === "container") {
        // initialize all the event listeners for the correct elements within the container
        geocoder.onAdd(container);
      } else {
        // returns the input elements, which are then added to the requested html container
        const el = geocoder.onAdd(container);
  ...
  onAdd: function (element) {
    if (element && typeof element != 'string' && !(element instanceof HTMLElement)) {
      this._map = element;
    }
    ...
    if (element.dataset && element.dataset.geocoderTarget === "container") {
      ...
      this._inputEl = this.container.querySelector("input[type='text']");
      ...
      this._inputEl.addEventListener("keyup", this._onKeyUp);

      this._clearEl = container.querySelector("button[aria-label='Clear']");
      this._clearEl.addEventListener("click", this.clear);

      this._loadingEl = container.querySelector(".mapboxgl-ctrl-geocoder--icon.mapboxgl-ctrl-geocoder--icon-loading");

      if (this.options.enableGeolocation && this.geolocation.isSupport()) {
        this._geolocateEl = container.querySelector("button[aria-label='Geolocate']");
        this._geolocateEl.addEventListener("click", this._geolocateUser);
      }

      const typeahead = this._typeahead = new Typeahead(this._inputEl, [], {
        filter: false,
      ...
      const parentDraw = typeahead.list.draw;
      const footerNode = this._footerNode = getFooterNode();
      typeahead.list.draw = function () {
        parentDraw.call(this);

        footerNode.addEventListener("mousedown", this._onMouseDown);
        footerNode.addEventListener("mouseup", this._onMouseUp);
      ...
      if (this.options.enableGeolocation && this.geolocation.isSupport()) {
        ...
        const geolocateIcon = this.createIcon("geolocate", "<path d=\"M12.999 3.677L2.042 8.269c-.962.403-.747 1.823.29 1.912l5.032.431.431 5.033c.089 1.037 1.509 1.252 1.912.29l4.592-10.957c.345-.822-.477-1.644-1.299-1.299z\" fill=\"#4264fb\"/>");
      ...
      typeahead.list.draw = function () {
        ...
        footerNode.addEventListener("mousedown", () => {
          this.selectingListItem = true;
        });

        footerNode.addEventListener("mouseup", () => {
  ...
  onRemove: function () {
    if (this.container.dataset.geocoderTarget === "container") {
      this._inputEl.value = "";
      this.container.removeEventListener("mouseenter", this._unCollapse);
      this.container.removeEventListener("mouseleave", this._collapse);
      this._inputEl.removeEventListener("focus", this._unCollapse);
      this._inputEl.removeEventListener("blur", this._onBlur);
      this._inputEl.removeEventListener("keydown", debounce(this._onKeyDown, 200));
      this._inputEl.removeEventListener("paste", this._onPaste);
      this._inputEl.removeEventListener("change", this._onChange);
      this.container.removeEventListener("mouseenter", this._showButton);
      this.container.removeEventListener("mouseleave", this._hideButton);
      this._inputEl.removeEventListener("keyup", this._onKeyUp);
      this._clearEl.removeEventListener("click", this.clear);
      this._geolocateEl.removeEventListener("click", this._geolocateUser);
      this._footerNode.removeEventListener("mousedown", this._onMouseDown);
      this._footerNode.removeEventListener("mouseup", this._onMouseUp);
    } else {
  ...
  _setInputValue: function (value) {
    ...
    setTimeout(() => {
      ...
      const valueLength = this._inputEl.value.length;
      this._inputEl.setSelectionRange(valueLength, valueLength);
    }, 1);
  },
